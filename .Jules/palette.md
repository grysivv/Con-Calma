...
## 2026-06-24 - Hiding standalone decorative icons
**Learning:** Standalone decorative `Image(systemName:)` elements are read by VoiceOver using unhelpful literal icon names, adding noise for accessibility users.
**Action:** Apply `.accessibilityHidden(true)` to standalone decorative `Image(systemName:)` elements (e.g., visual cues like 'hand.tap.fill' or 'brain.head.profile') to prevent VoiceOver from reading them.
