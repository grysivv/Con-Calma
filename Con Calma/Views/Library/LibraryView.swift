...
struct AddFlashcardView: View {
    ...
    var duplicateStatus: DuplicateStatus {
        ...
        guard !cleanFront.isEmpty else { return .none }

        let cleanExample = normalize(example)
        let hasContext = !cleanExample.isEmpty

        for card in allCards {
            if normalize(card.front) == cleanFront {
                if !hasContext {
                    return .needsContext
                } else if normalize(card.example ?? "") == cleanExample {
        ...
        NavigationStack {
            Form {
                Section(header: Text("Główne")) {
                    ...
                    if duplicateStatus == .needsContext {
                        Text("To włoskie słówko już istnieje. Dodaj przykładowe zdanie poniżej, aby zapisać jego inne znaczenie.")
                            .font(.caption)
                            .foregroundColor(.red)
                    } else if duplicateStatus == .exactDuplicate {
                        Text("Włoskie słówko z identycznym zdaniem już istnieje w bazie. Zmień kontekst lub słówko.")
                            .font(.caption)
                            .foregroundColor(.red)
                    }
                }
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    ...
                    .disabled(front.trimmingCharacters(in: .whitespaces).isEmpty || back.trimmingCharacters(in: .whitespaces).isEmpty || duplicateStatus != .none)
                }
            }
            .onAppear {
                isFrontFocused = true
